...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(prev->nHeight <= 1) txNew.vout[0].nValue = 92250000 * COIN;
    if(prev->nHeight >= 2) txNew.vout[0].nValue = 500 * COIN;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("unityMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("unityMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("unityMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("unity-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running unityMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),

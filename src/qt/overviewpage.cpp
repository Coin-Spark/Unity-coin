...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeunityAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeunityAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeunityAmount = strAnonymizeunityAmount.remove(strAnonymizeunityAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeunityAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeunityAmount * COIN) nMaxToAnonymize = nAnonymizeunityAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeunityAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeunityAmount));
        strAnonymizeunityAmount = strAnonymizeunityAmount.remove(strAnonymizeunityAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeunityAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {

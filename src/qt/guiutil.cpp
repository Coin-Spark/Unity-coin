...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a unity address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no unity: URI
...
{
    // Convert unity:// to unity:
    //
    //    Cannot handle this later, because unity:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open unity.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "unity.lnk";
}
...
{
    return GetAutostartDir() / "unity.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a unity.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=unity\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the unity app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add unity app to startup item list

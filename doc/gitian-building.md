...
*Setup instructions for a gitian build of unity using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the unity
...
Clone the git repositories for unity and gitian and then checkout the unity version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/unity-crypto/unity.git
cd unity
...
Follow the instructions in [doc/release-process.md](release-process.md) in the unity repository
...
To build unity (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the unity repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/unity/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/unity-crypto/unity
...
For example:
```bash
URL=https://github.com/crowning-/unity.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit unity=${COMMIT} --url unity=${URL} ../unity/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit unity=${COMMIT} --url unity=${URL} ../unity/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit unity=${COMMIT} --url unity=${URL} ../unity/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/unity-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/unity-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/unity-build.assert
```
...
[unity/gitian.sigs](https://github.com/unity-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
